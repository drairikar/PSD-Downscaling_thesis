...
class AFNOWrapper(pl.LightningModule):
    def __init__(self, args):
        ...
        ##AFNO model
        self.model = AFNO(
            inp_shape = [self.img_shape_y, self.img_shape_x],
            in_channels = self.img_in_channels,
            out_channels = self.img_out_channels,
            patch_size=getattr(args, 'afno_patch_size', [8, 8]),
            embed_dim=getattr(args, 'afno_embed_dim', 128),
            depth = getattr(args, 'afno_depth', 4),
            mlp_ratio=getattr(args, 'afno_mlp_ratio', 4.0),
            drop_rate=getattr(args, 'afno_drop_rate', 0.0),
            num_blocks=getattr(args, 'afno_num_blocks', 8),
            sparsity_threshold=getattr(args, 'afno_sparsity_threshold', 0.01),
            hard_thresholding_fraction=getattr(args, 'afno_hard_thresholding_fraction', 1.0),
    ...
    def forward(
        ...
        **model_kwargs : dict,
        ...
        """
        Forward pass of AFNO
        """
        if img_lr is None:
            raise ValueError("LR image is required for AFNO")
